...
typedef struct space {
    ...
    unsigned int alloc_head;
    unsigned int alloc_tail;
} space;
...
int swap_out() {
    ...
    /* swap space 값 변경 */
    ku_mmu_swap.alloc_head = ku_mmu_swap.free_head;
    ku_mmu_swap.free_head = ku_mmu_swap.free_and_alloc[ku_mmu_swap.free_head].next_index;
    
    /* swap out된 pte 값 변경 */
    ku_pte *swap_out_pte = ku_mmu_pmem.free_and_alloc[ku_mmu_pmem.alloc_tail].rmap;
    ...
    ku_mmu_pmem.free_head = ku_mmu_pmem.alloc_tail; /* free list의 head가 alloc list의 tail 값이 됨 */
    ku_mmu_pmem.alloc_tail = ku_mmu_pmem.free_and_alloc[ku_mmu_pmem.alloc_tail].next_index;
    return 0;
}
...
int swap_in(ku_pte *accesed_pte) {
    ...
    node* taking_node = &ku_mmu_swap.free_and_alloc[take_back];
    taking_node -> rmap = NULL; // free 된 swap을 free list에 insert
    ku_mmu_swap.free_and_alloc[taking_node -> prev_index].next_index = taking_node -> next_index;
    ku_mmu_swap.free_head = take_back;
...
int map_on_pmem(ku_pte *accesed_pte) {
    ...
    if(ku_mmu_pmem.alloc_tail == 0) {
        ku_mmu_pmem.alloc_head = 1;
        ku_mmu_pmem.alloc_tail = 1;
    } else {
        ku_mmu_pmem.alloc_head = ku_mmu_pmem.free_and_alloc[ku_mmu_pmem.alloc_head].next_index;
    }
    ku_mmu_pmem.free_and_alloc[ku_mmu_pmem.alloc_head].rmap = accesed_pte; // 할당된 pte reverse mapping
    ku_mmu_pmem.free_head = ku_mmu_pmem.free_and_alloc[ku_mmu_pmem.free_head].next_index; // free head 변경
    return 0;
}
...
int ku_page_fault(char pid, char va) {
    ...
    if(target_pte -> PFN == 0x0 && target_pte -> unused_bit == 0x0) {
        if(map_on_pmem(target_pte)) return -1; // TODO pbit이 아닌 ubit 값이 갱신됨
    } else {
        if(swap_in(target_pte)) return -1; 
    }
    return 0;
}